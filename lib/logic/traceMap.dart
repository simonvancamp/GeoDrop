...
class GeohashTree<V> {
  final GeohashTreeNode<V> _root = GeohashTreeNode(null, level: 0);
  final int maxDepth;
  int length = 0;
  ...
  List<V> getGeohashesByProximity(
      double latitude, double longitude, double radius,
      {int precision}) {
    int precision = precision ?? maxDepth;
    List<String> proximityGeohashes =
        createGeohashes(latitude, longitude, radius, precision);
  ...
  List<V> getGeohashes(String geohash, {int precision}) {
    int precision = precision ?? geohash.length;

    if (precision < 0) throw ArgumentError('Incorrect precision found');
    if (geohash.length < precision) {
      throw ArgumentError("geohash must be less than the precision");
    }
    if (precision > maxDepth) {
      throw ArgumentError("precision is higher than the max tree depth");
    }
    GeohashTreeNode<V> node = _getNode(_root, geohash.substring(0, precision));
    if (node == null) return null;
  ...
  List<V> _getValues(GeohashTreeNode<V> parent) {
    ...
    for (var node in parent.children.values) {
      values.addAll(_getValues(node));
    }
    return values;
  }

  List<String> _getGeohashes(GeohashTreeNode<V> parent) {
    List<String> geohashes = parent.value != null ? [parent.geohash] : [];
    if (parent.children == null) {
      return geohashes;
    }
  ...
  void addLatLng(double lat, double long, V v, {int precision}) {
    int precision = precision ?? maxDepth;
    String hashedKey = GeoHasher().encode(long, lat, precision: precision);
  ...
  V update(String geohash, V Function(V value) update,
      {V Function() ifAbsent}) {
    GeohashTreeNode<V> node = _getNode(_root, geohash);
    if (node == null) {
      if (ifAbsent == null) {
        throw (ArgumentError(
            "Geohash not found and ifAbsent() is not provided"));
      }
  ...
  /// in the tree then [ifAbsent] is called.
  V updateLatLng(double lat, double long, V Function(V value) updateValue,
      {int precision, V Function() ifAbsent}) {
    int precision = precision ?? maxDepth;
    String geohash = GeoHasher().encode(long, lat, precision: precision);
  ...
  void removeWhere(bool Function(String geohash, V value) f) {
    List<GeohashTreeNode<V>> nodes = _getNodes(_root);
    Iterable<GeohashTreeNode<V>> removeNodes =
        nodes.where((node) => f(node.geohash, node.value));
    for (var n in removeNodes) {
      remove(n.geohash);
    }
  }
  ...
  V remove(String geohash) {
    GeohashTreeNode<V> parentNode = _getNode(
        _root, geohash.substring(0, geohash.length - 1).substring(0, maxDepth));
    GeohashTreeNode<V> removedNode =
        parentNode.children.remove(geohash.substring(maxDepth, geohash.length));
    if (removedNode == null) {
      return null;
    }
    length -= 1;
  ...
  void forEach(void Function(String geohash, V value) f) {
    List<GeohashTreeNode> nodes = _getNodes(_root);
    for (var node in nodes) {
      f(node.geohash, node.value);
    }
  }
  ...
  void _putNode(GeohashTreeNode<V> parent, String geohash, V v,
      {int level = 0}) {
    String hashLetter = level == maxDepth
        ? geohash.substring(level)
        : geohash.substring(level, level + 1);
    ...
    if (level == maxDepth) {
      ...
      parent.children[hashLetter] =
          GeohashTreeNode(geohash, value: v, level: level);
      length = length + 1;
      return;
    }
  ...
  GeohashTreeNode<V> _getNode(GeohashTreeNode<V> parent, String geohash,
      {int level = 0}) {
